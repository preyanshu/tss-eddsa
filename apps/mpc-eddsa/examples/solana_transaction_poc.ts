...
export async function signAndSendSolanaTransaction(
    ...
    console.log('╔══════════════════════════════════════════════════════════════╗');
    console.log('║     Solana Transaction Signing POC                          ║');
    console.log('╚══════════════════════════════════════════════════════════════╝\n');
    ...
    // ============================================
    console.log('┌──────────────────────────────────────────────────────────────┐');
    console.log('│ STEP 1: Connect to Solana Network                           │');
    console.log('└──────────────────────────────────────────────────────────────┘\n');
    ...
    const signingAggregatePubKeyBytes = Buffer.from(keygenResult.aggregatePublicKey.bytes);
    if (signingAggregatePubKeyBytes.length !== 32) {
        throw new Error(`Invalid signing public key length: expected 32 bytes, got ${signingAggregatePubKeyBytes.length}`);
    }
    ...
    console.log(`  MPC Public Key: ${signingAggregatePubKeyBytes.toString('hex').substring(0, 64)}...\n`);

    // ============================================
    // STEP 3: Fund Sender Account
    // ============================================
    console.log('┌──────────────────────────────────────────────────────────────┐');
    console.log('│ STEP 3: Fund Sender Account                                   │');
    console.log('└──────────────────────────────────────────────────────────────┘\n');
    
    try {
        console.log('Requesting airdrop...');
        const airdropSignature = await connection.requestAirdrop(senderPubkey, 2 * LAMPORTS_PER_SOL);
    ...
    // ============================================
    console.log('┌──────────────────────────────────────────────────────────────┐');
    console.log('│ STEP 5: Sign Transaction with MPC                           │');
    console.log('└──────────────────────────────────────────────────────────────┘\n');
    ...
    console.log(`  Transaction message length: ${transactionMessage.length} bytes\n`);
    
    // Sign transaction message using the keys we already generated
    const signingSession = coordinator.startSigning(transactionMessage, signingParties);
    ...
    for (let i = 0; i < signingParties.length; i++) {
        ...
        const eph = services[partyId].startEphemeralKeyGeneration(transactionMessage, signingSession.signingParties[i]);
    ...
    for (let i = 0; i < signingParties.length; i++) {
        const partyId = signingParties[i];
        const localSigResult = services[partyId].computeLocalSignature(transactionMessage, ephSharedKeys[i]);
    ...
    console.log(`✓ Transaction signed with MPC (${signingParties.length} parties)`);
    ...
    // ============================================
    console.log('┌──────────────────────────────────────────────────────────────┐');
    console.log('│ STEP 6: Convert to Solana Signature Format                  │');
    console.log('└──────────────────────────────────────────────────────────────┘\n');
    ...
    if (rBytes.length !== 32 || sBytes.length !== 32) {
        throw new Error(`Invalid signature component lengths: R=${rBytes.length}, s=${sBytes.length}`);
    }
    ...
    console.log(`  Solana signature: ${solanaSignature.toString('hex').substring(0, 64)}...`);
    ...
    // ============================================
    console.log('┌──────────────────────────────────────────────────────────────┐');
    console.log('│ STEP 7: Add Signature to Transaction                        │');
    console.log('└──────────────────────────────────────────────────────────────┘\n');
    ...
    console.log(`  Signature (first 32 bytes): ${Buffer.from(signatureArray.slice(0, 32)).toString('hex').substring(0, 32)}...\n`);
    ...
    try {
        ...
        if (confirmation.value.err === null) {
            ...
            console.log(`  View on Solana Explorer: https://explorer.solana.com/tx/${txSignature}?cluster=custom&customUrl=http://localhost:8899\n`);
            
            // Get transaction details
            const txStatus = await connection.getTransaction(txSignature, { commitment: 'confirmed' });
            if (txStatus && txStatus.meta && txStatus.meta.err === null) {
                ...
                if (txStatus.meta.postBalances && txStatus.meta.preBalances) {
                    const balanceChange = txStatus.meta.postBalances[0] - txStatus.meta.preBalances[0];
                    console.log(`  Balance change: ${balanceChange / LAMPORTS_PER_SOL} SOL`);
                }
            }
        } else {
        ...
        if (error.message.includes('signature')) {
            console.log('  Note: Verify the MPC signature matches the transaction message');
        }
        throw error;
    }
}
