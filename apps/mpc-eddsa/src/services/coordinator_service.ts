...
export class CoordinatorService {
     ...
     */
    registerParty(partyId: string, publicKey: SerializableBigInt): PartyRegistrationResponse {
        if (!this.session) {
            throw new SessionError('Session not initialized. Call startKeyGeneration() first');
        }
        ...
        if (!party || party.protocolIndex === undefined) {
            throw new PartyError(`Party ${partyId} not found or protocolIndex not set`);
        }
        return {
     ...
     */
    collectCommitments(partyCommitments: Array<{ partyId: string; commitment: SerializableBigInt; blindFactor: SerializableBigInt }>): CommitData {
        // partyCommitments: [{ partyId, commitment, blindFactor }, ...]
        this.commitments = [];
        this.blindFactors = [];
        
        // Sort by partyId to match party order
        const sorted = partyCommitments.sort((a, b) => a.partyId.localeCompare(b.partyId));
        
        // Normalize to Buffer for internal storage
        for (const commit of sorted) {
            this.commitments.push(serialization.normalizeBytesToBuffer(commit.commitment));
            this.blindFactors.push(serialization.normalizeBytesToBuffer(commit.blindFactor));
        }
        ...
        return {
            ...
            blindFactors: this.blindFactors.map(bf => serialization.serializeForHttp(bf)),
            publicKeys: this.parties.map(p => serialization.serializeForHttp(p.publicKey)),
            commitments: this.commitments.map(c => serialization.serializeForHttp(c)),
     ...
     */
    collectShares(partyShares: Array<{ partyId: string; vss: VSSScheme; secretShares: SecretShare[] }>): ShareData {
        // partyShares: [{ partyId, vss, secretShares }, ...]
        this.allVssSchemes = [];
        this.allSecretShares = [];
        
        // Sort by partyId
        const sorted = partyShares.sort((a, b) => a.partyId.localeCompare(b.partyId));
        
        // Normalize secretShares to Buffer for internal storage
        for (const share of sorted) {
            this.allVssSchemes.push(share.vss);
            this.allSecretShares.push(share.secretShares.map(s => serialization.normalizeBytesToBuffer(s)));
        }
        ...
        for (let i = 0; i < shareCount; i++) {
            ...
            result[party.partyId] = {
                ...
                shareCount,
                publicKeys: this.parties.map(p => serialization.serializeForHttp(p.publicKey)),
                allSecretShares: serialization.serialize2DArrayForHttp(this.allSecretShares),
     ...
     */
    collectSharedKeys(partySharedKeys: Array<{ partyId: string; sharedKey: SharedKey }>): {
        ...
        // Sort by partyId
        const sorted = partySharedKeys.sort((a, b) => a.partyId.localeCompare(b.partyId));
        ...
        for (const sk of sorted) {
            this.sharedKeys.push({
                ...
                prefix: sk.sharedKey.prefix ? serialization.normalizeBytesToBuffer(sk.sharedKey.prefix) : undefined
            });
        }
        
        // Compute aggregate public key
        const { aggregatePublicKey } = MPCClient.aggregateKey(this.sharedKeys);
        const aggregatePublicKeyHex = Buffer.from(aggregatePublicKey.bytes).toString('hex');
        ...
        return {
            ...
            sharedKeys: this.sharedKeys.map(sk => ({
                ...
                prefix: sk.prefix ? serialization.serializeForHttp(sk.prefix) : undefined
            }))
        };
    }
     ...
     */
    startSigning(message: Buffer | number[] | string, signingParties: string[]): {
        message: Buffer;
        ...
        if (signingParties.length < this.session.threshold) {
            validateSigningParties(signingParties.length, this.session.threshold, this.session.totalParties);
        }
        ...
        this.signingPartyIndices = signingParties.map(pid => {
     ...
     */
    collectEphemeralKeys(partyEphKeys: Array<{ partyId: string; ephR: SerializableBigInt }>): {
        ephRPoints: SerializableBigInt[];
        signingPartyIndices: number[];
    } {
        // partyEphKeys: [{ partyId, ephR }, ...]
        const ephRPoints = [];
        const sorted = partyEphKeys.sort((a, b) => a.partyId.localeCompare(b.partyId));
     ...
     */
    collectEphemeralCommitments(partyEphCommitments: Array<{ partyId: string; commitment: SerializableBigInt; blindFactor: SerializableBigInt }>): {
        ...
        const sorted = partyEphCommitments.sort((a, b) => a.partyId.localeCompare(b.partyId));
     ...
     */
    collectEphemeralKeysAndCommitments(partyEphData: Array<{ partyId: string; ephR: SerializableBigInt; commitment: SerializableBigInt; blindFactor: SerializableBigInt }>): EphemeralCommitData {
        // partyEphData: [{ partyId, ephR, commitment, blindFactor }, ...]
        const sorted = partyEphData.sort((a, b) => a.partyId.localeCompare(b.partyId));
        ...
        for (const data of sorted) {
            ...
            ephCommitments.push(serialization.normalizeBytesToBuffer(data.commitment));
            ephBlindFactors.push(serialization.normalizeBytesToBuffer(data.blindFactor));
        }
        ...
        return {
            ...
            ephCommitments: ephCommitments.map(c => serialization.serializeForHttp(c)),
            ephBlindFactors: ephBlindFactors.map(bf => serialization.serializeForHttp(bf)),
            signingPartyIndices: this.signingPartyIndices
        };
    }
     ...
     */
    collectEphemeralShares(partyEphShares: Array<{ partyId: string; vss: VSSScheme; secretShares: SecretShare[] }>): EphemeralShareData {
        const allEphVssSchemes = [];
        const allEphSecretShares = [];
        const sorted = partyEphShares.sort((a, b) => a.partyId.localeCompare(b.partyId));
        
        // Normalize secretShares to Buffer for internal storage
        for (const share of sorted) {
            allEphVssSchemes.push(share.vss);
            allEphSecretShares.push(share.secretShares.map(s => serialization.normalizeBytesToBuffer(s)));
        }
        
        this.allEphVssSchemes = allEphVssSchemes;
        this.allEphSecretShares = allEphSecretShares;
        
        if (!this.signingPartyIndices || !this.session || !this.ephRPoints || !this.allEphSecretShares) {
        ...
        for (let i = 0; i < this.signingPartyIndices.length; i++) {
            ...
            result[party.partyId] = {
                ...
                allEphSecretShares: serialization.serialize2DArrayForHttp(this.allEphSecretShares),
     ...
     */
    collectLocalSignatures(partyLocalSigs: Array<{ partyId: string; localSig: LocalSignature }>): SignatureResult {
        // partyLocalSigs: [{ partyId, localSig }, ...]
        const localSigs = [];
        const sorted = partyLocalSigs.sort((a, b) => a.partyId.localeCompare(b.partyId));
        ...
        if (!this.signingPartyIndices || !this.signingSession || !this.allVssSchemes || !this.allEphVssSchemes || !this.ephSharedKeys || !this.aggregatePublicKey) {
            throw new StateError('Required state not initialized');
        }
        ...
        if (this.ephSharedKeys.length > 0) {
            ...
            if (aggregateR && aggregateR.bytes) {
                if (!Array.isArray(aggregateR.bytes)) {
                    aggregateR.bytes = Array.from(aggregateR.bytes, b => typeof b === 'number' ? b : Number(b));
                } else {
                    aggregateR.bytes = aggregateR.bytes.map(b => typeof b === 'number' ? b : Number(b));
                }
            }
        }
        ...
        const messageArray = serialization.messageToArray(this.signingSession.message);
        ...
        const signatureHex = Buffer.from(sigR.bytes).toString('hex') + Buffer.from(sigS.bytes).toString('hex');
        ...
        return {
            ...
            signatureHex,
            aggregatePublicKey: serialization.serializeForHttp(this.aggregatePublicKey),
            aggregatePublicKeyHex: Buffer.from(this.aggregatePublicKey.bytes).toString('hex'),
            isValid
        };
    }
    ...
    getKeyConstructionDataHttp(): ShareData {
        if (!this.session || !this.allSecretShares || this.allSecretShares.length === 0) {
            throw new StateError('Shares not collected. Make sure collect-shares was called first.');
        }
    ...
    getEphCommitDataForSharesHttp(): EphemeralCommitData {
        if (!this.ephRPoints || this.ephRPoints.length === 0 || !this.signingPartyIndices) {
            throw new StateError('Ephemeral keys not collected');
        }
        return {
            ephRPoints: this.ephRPoints.map(r => serialization.serializeForHttp(r)),
            ephCommitments: (this.ephCommitments || []).map(c => serialization.serializeForHttp(c)),
            ephBlindFactors: (this.ephBlindFactors || []).map(bf => serialization.serializeForHttp(bf)),
            signingPartyIndices: this.signingPartyIndices
        };
    }
    ...
    getEphKeyConstructionDataHttp(): EphemeralShareData {
        if (!this.allEphSecretShares || this.allEphSecretShares.length === 0) {
            throw new StateError('Ephemeral shares not collected. Make sure collect-ephemeral-shares was called first.');
        }
        if (!this.ephRPoints || this.ephRPoints.length === 0) {
            throw new StateError('Ephemeral R points not available. Make sure collect-ephemeral-keys was called first.');
        }
